...
if buildIndex:
	...
	for i in range(51,100): #the range of documents to be parsed
		count = 0
		writer = ix.writer()

		### --- IMPORTANT --- ###
		fName = "t2/tweets" + str(i) + ".txt" #this path must point to wherever you have the tweet files. So you will need to change "../Part 1/tweets" to somethin else, depending on where you have the tweets stored.
		print "loading " + fName
		f = open(fName, 'rb')
		try:
			...
			while True:
				try:
					...
					break
				count += 1
				if count>100:#this limits the indexer to the first 1000 tweets of each file. I did this to make the indexer build faster. You can make this larger but the index will take longer to build and will take u more space.
					break
		finally:	
			f.close()
